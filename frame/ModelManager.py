...
class ModelManager():
    ...
    def test(self):
        ...
        if self.mode == 'train-test' or self.mode == 'cross validation':
            ...
            if test_dataloader is not None:
                test_performance, avg_test_loss, _, _, _, _ = self.__SL_test(test_dataloader)
    ...
    def __cross_validation(self, train_dataloader, valid_dataloader):
        ...
        for epoch in range(1, self.config.epoch + 1):
            ...
            if epoch % self.config.interval_valid == 0:
                valid_performance, avg_test_loss, _, _, _, _ = self.__SL_test(valid_dataloader)
    ...
    def __SL_train(self, train_dataloader, test_dataloader):
        ...
        if self.config.adversarial == True and self.config.model == 'FusionDNAbert':
            ...
            FGMModel = FGM.FGM(self.model, 'bertone.bert.embeddings.word_embeddings',
                               'berttwo.bert.embeddings.word_embeddings')
        ...
        for epoch in range(1, self.config.epoch + 1):
            ...
            for batch in train_dataloader:
                data, label = batch
                # print(label)
                logits, _, infoloss = self.model(data)
                train_loss =0.3*self.__get_loss(logits, label)+0.7*infoloss
                self.optimizer.zero_grad()
                train_loss.backward()
                if self.config.adversarial == True and self.config.model == 'FusionDNAbert':
                    FGMModel.attack()  # embedding被修改了
                    # self.optimizer.zero_grad()  # 如果不想累加梯度，就把这里的注释取消
                    logitsattack, _,infoloss = self.model(data)
                    attack_train_loss = 0.3*self.__get_loss(logitsattack, label)+0.7*infoloss
    ...
    def __SL_test(self, dataloader):
        ...
        with torch.no_grad():
            for batch in dataloader:
                data, label = batch
                logits, representation,infoloss = self.model(data)
                avg_test_loss += 0.5*self.__get_loss(logits, label)+0.5*infoloss
